...
describe("DropdownMenu", () => {
  it(
    "opens on trigger interaction and shows menu items",
    async () => {
      render(
        <DropdownMenu>
      ...
      expect(await screen.findByRole("menuitem", { name: "Edit" }, { timeout: 20000 })).toBeInTheDocument();
  ...
  it(
    "calls onSelect when a menu item is chosen",
    async () => {
      ...
      openMenu(screen.getByRole("button", { name: "Actions" }));
      const item = await screen.findByRole("menuitem", { name: "Edit" }, { timeout: 20000 });
  ...
  it(
    "the destructive prop applies distinct styling from a regular item",
    async () => {
      render(
        <DropdownMenu>
          <DropdownMenuTrigger>Actions</DropdownMenuTrigger>
      ...
      openMenu(screen.getByRole("button", { name: "Actions" }));
      const edit = await screen.findByRole("menuitem", { name: "Edit" }, { timeout: 20000 });
